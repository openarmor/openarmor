...
brief: These tests will check if the 'openarmor-authd' daemon correctly handles the enrollment requests
       from agents with pre-existing IP addresses or names. The 'openarmor-authd' daemon can automatically
...
def test_ossec_authd_agents_ctx(test_configuration, test_metadata, set_wazuh_configuration, truncate_monitored_files,
    ...
    '''
    description:
        Check if when the 'openarmor-authd' daemon receives an enrollment request from an agent
    ...
    input_description:
        ...
        which includes configuration settings for the 'openarmor-authd' daemon.

    expected_output:
        - r'Accepting connections on port 1515' (When the 'openarmor-authd' daemon is ready to accept enrollments)
