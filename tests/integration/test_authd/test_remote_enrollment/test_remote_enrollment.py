...
brief: These tests will check if the 'remote enrollment' option of the 'openarmor-authd' daemon
       settings is working properly. The 'openarmor-authd' daemon can automatically add
...
def test_remote_enrollment(test_configuration, test_metadata, set_wazuh_configuration,
    ...
    '''
    description:
        Checks if the 'openarmor-authd' daemon remote enrollment is enabled/disabled according
    ...
    expected_output:
        - r'Accepting connections on port 1515. No password required.' (When the 'openarmor-authd' daemon)
