...
brief: The 'openarmor-analysisd' daemon receives the log messages and compares them to the rules.
       ...
       Specifically, these tests will check if the 'openarmor-analysisd' daemon correctly handles
...
def test_event_messages(test_metadata, configure_local_internal_options, configure_sockets_environment_module,
    ...
    '''
    description: Check if when the 'openarmor-analysisd' daemon socket receives a message with
                 a file modification-related event, it generates the corresponding alert
                 that sends to the 'openarmor-db' daemon socket.
