...
brief: The 'openarmor-analysisd' daemon receives the log messages and compares them to the rules.
       It then creates an alert when a log message matches an applicable rule.
       Specifically, these tests will check if the 'openarmor-analysisd' daemon handles correctly
...
def test_error_messages(test_metadata, configure_local_internal_options, configure_sockets_environment_module,
    ...
    '''
    description: Check if when the 'openarmor-analysisd' daemon socket receives a message with an invalid event,
                 it generates the corresponding error that sends to the 'openarmor-db' daemon socket.
