...
SOCKET_PATH_DAEMONS_MAPPING = {'/var/ossec/queue/sockets/remote': 'openarmor-remoted',
                               '/var/ossec/queue/sockets/analysis': 'openarmor-analysisd'}
DAEMON_SOCKET_PATHS_MAPPING = {'openarmor-remoted': '/var/ossec/queue/sockets/remote',
                               'openarmor-analysisd': '/var/ossec/queue/sockets/analysis'}
...
def test_get_daemons_stats(mock_get_daemons_stats_socket):
    ...
    response = stats.get_daemons_stats(['openarmor-remoted', 'openarmor-analysisd', 'openarmor-db'])
...
def test_get_daemons_stats_ko():
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['openarmor-remoted'])
...
@pytest.mark.parametrize('daemons_list, expected_daemons_list', [
    ([], ['openarmor-remoted', 'openarmor-analysisd']),
    (['openarmor-remoted'], ['openarmor-remoted']),
    (['openarmor-remoted', 'openarmor-analysisd'], ['openarmor-remoted', 'openarmor-analysisd'])
])
