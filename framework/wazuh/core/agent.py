...
class Agent:
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        str
            Message generated by Wazuh.
        """
        # Check that openarmor-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except WazuhInternalError as e:
            # openarmor-authd is not running due to a problem with /proc availability
            raise WazuhError(1726, extra_message=str(e))

        if manager_status.get('openarmor-authd') != 'running':
            # openarmor-authd is not running
