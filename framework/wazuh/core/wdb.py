...
class AsyncWazuhDBConnection:
    ...
    async def _send(self, msg, raw=False):
        """Format and send message to openarmor-db socket without blocking event loop.

        Parameters
        ----------
        msg : str
            Message to be sent to openarmor-db.
        raw : bool
            If `True`, the status message from openarmor-db is included in the response.

        Returns
        -------
        str, list
            Result for the request sent to openarmor-db.
        """
        try:
    ...
    async def run_wdb_command(self, command):
        ...
        ----------
        command : str
            Command to be executed inside openarmor-db
...
class WazuhDBConnection:
    ...
    def __init__(self, request_slice=500):
        ...
        ----------
        request_slice : int
            Maximum number of items to request from openarmor-db on the first call.
        """
        self.socket_path = common.WDB_SOCKET
    ...
    def _send(self, msg: str, raw: bool = False) -> dict:
        ...
        WazuhInternalError(2009)
            Pagination error. Response from openarmor-db was over the maximum socket buffer size.
        WazuhError(2003)
    ...
    def loads(string: str) -> dict:
        ...
        ----------
        string : str
            String response from `openarmor-db`. It must be a dumped JSON.
    ...
    def delete_agents_db(self, agents_id: List[str]) -> dict:
        """Delete agents db through openarmor-db service.
    ...
    def send(self, query: str, raw: bool = True) -> Union[str, dict]:
        ...
        ----------
        query : str
            Query to be executed in openarmor-db.
        raw : bool
