...
function install_wazuh(){
  ...
  eval "${*} apt install -y ./openarmor-agent_${VERSION}-0.commit${SHA}_amd64.deb > /dev/null 2>&1"
  
}

function remove_wazuh () {

  apt purge -y openarmor-agent > /dev/null 2>&1
...
echo "Download package: https://s3.us-west-1.amazonaws.com/packages-dev.wazuh.com/warehouse/pullrequests/${MAJOR}.${MINOR}/deb/var/openarmor-agent_${VERSION}-0.commit${SHA}_amd64.deb"
wget "https://s3.us-west-1.amazonaws.com/packages-dev.wazuh.com/warehouse/pullrequests/${MAJOR}.${MINOR}/deb/var/openarmor-agent_${VERSION}-0.commit${SHA}_amd64.deb" > /dev/null 2>&1
