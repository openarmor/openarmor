...
static const size_t c_nBlockLen = 1;
static const size_t c_nHashLen = 4;
...
static const uint32_t g_nInitValue = uint32_t(-1); // Must be 0 or -1

inline const uint32_t* getTable()
{
	static const uint32_t Table[0x100] =
	{
...
//
//
static inline uint32_t transform(const uint8_t bSymbol, uint32_t nChecksum)
{
...
//
//
inline void finalize(Context* pCtx, uint32_t& Digest)
{
