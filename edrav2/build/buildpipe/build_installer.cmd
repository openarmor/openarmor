...
set product=edrav2
if exist "c:\Program Files (x86)\Microsoft Visual Studio\%vs_ver%\Community\MSBuild\current\Bin\MSBuild.exe" (
	set msbuild="c:\Program Files (x86)\Microsoft Visual Studio\%vs_ver%\Community\MSBuild\current\Bin\MSBuild.exe"
) else set msbuild="c:\Program Files (x86)\Microsoft Visual Studio\%vs_ver%\Community\MSBuild\15.0\Bin\MSBuild.exe"
set vsdevcmd="C:\Program Files (x86)\Microsoft Visual Studio\%vs_ver%\Community\Common7\Tools\VsDevCmd.bat"
set CL=/Zm500
rem ========SFTP settings============
...
echo.Place signed binaries into folders "%~dp0_Build_\edrav2\out\bin\win-Release-Win32" and "%~dp0_Build_\edrav2\out\bin\win-Release-x64"
pause
...
git clone -n %git_url% -- >>"%~dp0Logs\git.log" 2>&1 || exit /b 1
cd "%~dp0_Build_\%product%"
git checkout %tag% >>"%~dp0Logs\git.log" 2>&1 || exit /b 1
